...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="",
                          cuisines="", limit=5, sort='rating', order="desc", start_offset=0):
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines) + "&sort="
                          + sort + "&order=" + order + "&start=" + str(start_offset), headers=headers).content).decode(
            "utf-8")
        return r
